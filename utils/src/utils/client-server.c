...
void enviar_pcb_contexto_motivo(int socket_destino, pcb_t *pcb_a_enviar, char *motivo)
{
	...
	agregar_a_paquete(paquete_pcb, motivo, string_length(motivo) + 1);
...
void *recibir_buffer_pcb_motivo(int *size, int socket_cliente)
{
	...
	pcb_deserealizado->pcb = list_get(lista,0);
	pcb_deserealizado->pcb->reg_generales = list_get(lista,1);
	pcb_deserealizado->motivo = list_get(lista,2);
